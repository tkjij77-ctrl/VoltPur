...
### Build 71 — استقرار تشغيلي: قاطع دائرة + ملف استرجاع + حجر صحي للـ config
**الفجوات المؤكدة من فحص الكود** (كما وردت في خطة الاستقرار):
1. `VoltPurGuard` كان يعدّ الفشل **ولا يوقف** الموديول أبدًا ⇒ موديول يفشل كل tick يظل يعمل للأبد.
2. `VoltPurConfig.init()` كان يكتفي بتحذير عند فشل قراءة `voltpur.yml` ⇒ الملف يظل يفشل كل إقلاع.
3. لا شيء يُكتب بجانب الجار يقود المشغّل لو البناء الجديد **لم يُقلع** (VoltPur نفسه لا يعمل حينها).

**ما نُفِّذ:**
- **قاطع دائرة** (`guard.circuit-breaker.enabled` افتراضي true · `threshold` افتراضي 8): بعد ٨ فشل متتالي يُعلَّق الموديول، وتُحتسب الاستدعاءات اللاحقة كـ `short-circuited`، وسطر SEVERE يشرح ويشير إلى `/voltpur reload`؛ نجاح واحد يُصفِّر العدّاد، والأعداد التراكمية للفشل تُحفظ للمصداقية. **موديولات المراقبة مستثناة** (ModuleGuard/TPSMonitor/WorldStability): تُسجَّل تحذيرًا شديدًا لكنها تستمر.
- **`RECOVERY.txt`** يُكتب بجانب الجار عند كل تحديث وعند كل رجوع: البناء والبصمة، اسم الجار السابق، وأمر `cp` الحرفي للاسترجاع — كل شيء قابل للقراءة بـ `cat`.
- **حجر صحي للـ config**: الملف المعطوب يُنقل إلى `voltpur.yml.broken-<stamp>` (لا يُحذف)، مع رسالة واضحة، والسيرفر يكمل على الافتراضيات.

**تحقّق:** `tools/verify/run-verify.sh` ⇒ **136 PASS / 0 FAIL** (٢٥ تأكيدًا جديدًا: ١٣ للقاطع + ٧ لملف الاسترجاع + ٥ للحجر الصحي)، وسطور اللوج الفعلية للتعليق والاستثناء ظاهرة في مخرجات الاختبارات.

**تصحيح ذاتي مهم:** أثناء اختبار القاطع حيًّا ادّعيتُ أن موديول النسخ الاحتياطي لا يُبلّغ الـ Guard، فأضفتُ تسجيلًا ثانيًا — ثم كشف الاختبار الحي أن **الادّعاء خاطئ** (`runBackup()` يُبلّغ الـ Guard فعلًا)، وأن الإضافة تُنتج **عدًّا مزدوجًا يُفتح القاطع عند نصف الحدّ**. أُعيد التغيير، والكود الأصلي كان سليمًا. الدرس المتكرر: **الاختبار الحي يحكم على الافتراض، لا العكس.**
**ملاحظة تشغيلية:** الحد الأدنى لفترة النسخ الاحتياطي المجدول هو **٥ دقائق** (`Math.max(5, interval-minutes)`) — يهم عند تصميم اختبارات كهذه.
...
- الرجوع **يرفض** نسخة بصمتها غير مطابقة (`MISMATCH`) ويشرح السبب دون تغيير أي ملف؛ والنسخ القديمة بلا بصمة تُستعمل مع تحذير صريح بأن سلامتها غير معروفة.
- كل نسخة جديدة تُسجَّل بصمتها في `<jar>.backups.sha256` (ملف بيان واحد).
- `update.keep-backups` (افتراضي **0 = لا يُحذف شيء**) لتدوير النسخ اختياريًا.

**درس من اختبار حي (مهم):** أول تصميم استخدم ملف بصمة لكل نسخة بجانبها (`.bak-<stamp>.sha256`). عند اختبار الرجوع بالجار **المنشور** Build 69، التقطت glob القديمة `server.jar.bak-*` ملف البصمة نفسه و«أعادت استرجاعه» كأنه `server.jar` (البصمة الناتجة `fcb1b9f2…`؛ لا 68 ولا 69). لذلك صُمِّم البيان باسم لا يطابق أي glob قديم — وهو ما يحفظ التوافق مع البناءات الأقدم.
...
**ملاحظة صدق عن الحاجز نفسه:** أول تشغيل للـ smoke test (Run #83) **فشل** — لكن ليس لعيب في المنتج: السيرفر أقلع `Done (12.979s)` بنجاح، والفشل كان في شيكٍ كتبتُه ضيّقًا جدًا (قَبِل صياغة `upper suggestion:` فقط، بينما حكم الهيب على runner يمتلك 16GB كان `conservative` بصياغة مختلفة). أُصلح الشيك ليقبل الصياغات الثلاث (كلها تحتوي `-Xmx<رقم>M`) وأُعيد التحقق منه على **لوج CI نفسه** وعلى **إقلاع محلي جديد**. الدرس: الحاجز كشف خطأ الحاجز — وهذا هو عملُه.
