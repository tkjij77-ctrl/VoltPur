...
→ compile OK (62 classes, JDK 21 أو 25) · core checks: 54 PASS · command checks: 85 PASS (139 total)
→ RESULT: PASS
```
الـ harness الجديد (`tools/verify/`) يترجم ملفات VoltPur فعليًا مقابل stubs للواجهة، ويشغّل **139 تأكيدًا سلوكيًا** (الافتراضات الآمنة، حفظ كومنتات `server.properties`، قائمة discord المسموح بها، عدّادات Guard، فحص روابط SSRF، تحقق الأرشيف، قيمة SHA-256 معروفة).
...
| 32 | **موديول النسخ الاحتياطي يُبلّغ عن الفشل في اللوج فقط ولا يبلّغ الـ Guard** ⇒ `/voltpur modules` يعرضه سليمًا بينما **كل** نسخة مجدولة تفشل (نفس العمى الذي أُنشئ الـ Guard لإزالته) | `recordScheduledResult(result)` يُسجّل الفشل في العدّادات ⇒ الموديول يظهر `FAILING`/`DISABLED`، والقاطع يوقفه إن استمر الفشل |
