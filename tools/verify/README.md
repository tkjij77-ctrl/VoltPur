...
```
tools/verify/run-verify.sh          # تشغيل كامل (ترجمة + 136 فحصًا سلوكيًا)
...
3. **السلوك** (136 تأكيدًا):
   ...
   - قاطع الدائرة: موديول يفشل ٨ مرات متتالية يُخرَج من جدوله تلقائيًا، وموديولات المراقبة مستثناة، و`/voltpur reload` يغلق القواطع.
   - `RECOVERY.txt` يُكتب بجانب الجار عند كل تحديث/رجوع (يعمل حتى لو الجار الجديد لم يُقلع).
   - `voltpur.yml` المعطوب يُنقل جانبًا (`*.broken-<stamp>`) ولا يُحذف أبدًا.
