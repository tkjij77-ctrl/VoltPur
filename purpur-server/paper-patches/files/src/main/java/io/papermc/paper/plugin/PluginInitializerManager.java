...
public class PluginInitializerManager {
    ...
    public static void load(OptionSet optionSet) throws Exception {
        ...
        // VoltPur start - plugin-pro/ loads BEFORE plugins/ (performance-first)
        // DirectoryProviderSource.INSTANCE creates the folder if missing and
        // only processes *.jar files (isValidFile), so a README.txt is ignored.
        io.papermc.paper.plugin.util.EntrypointUtil.registerProvidersFromSource(
            ...
            java.nio.file.Path.of("plugin-pro")
        );
        // VoltPur end
