...
public class VoltPurWorldCheck {
    ...
    private static void checkFiles() {
        Logger logger = Bukkit.getLogger();
        // Ensure the world folders exist in the root, like other server software.
        ensureWorldFolders();
    ...
    /**
     * Creates the world folders (world, world_nether, world_the_end) in the server
     * root if they do not exist, so they appear alongside the other files just like
     * any other server software. Also ensures each has a level.dat placeholder note
     * (the actual data is generated by Minecraft on load). This is real folder
     * creation - not just a check.
     */
    private static void ensureWorldFolders() {
        ...
        for (String w : worlds) {
            try {
                ...
                if (!dir.exists()) {
                    dir.mkdirs();
                    Bukkit.getLogger().info("[VoltPur-World] Created world folder: " + w + "/");
                }
            } catch (Exception e) {
                Bukkit.getLogger().warning("[VoltPur-World] Could not create " + w + ": " + e.getMessage());
            }
        }
    }
